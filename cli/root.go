// Package cli assembles the scryfall command tree from the scryfall
...
// NewApp assembles the kit application from the scryfall domain. The
...
//
// To add a command, declare it in scryfall/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := scryfall.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(scryfall.Domain{}).Register(app)
