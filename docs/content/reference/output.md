...
describes what users get. Pick a format with `-o`, or let scryfall choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
scryfall <command> -o table   # aligned columns for reading
scryfall <command> -o jsonl   # one JSON object per line, for piping
scryfall <command> -o json    # a single JSON array
scryfall <command> -o csv     # spreadsheet friendly
scryfall <command> -o tsv     # tab-separated
scryfall <command> -o url     # just the URL column
scryfall <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
scryfall <command> --fields id,title,url
```
...
```bash
scryfall <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
scryfall <command>            # a table, because this is a terminal
scryfall <command> | wc -l    # JSONL, because this is a pipe
```
